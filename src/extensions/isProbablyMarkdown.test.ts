...
describe("real Markdown is detected across languages", () => {
  const markdown: [string, string][] = [
    ...
    ["list after a label line", "Shopping list:\n\n- milk\n- eggs\n- bread"],
...
describe("structured non-Markdown is NOT treated as Markdown", () => {
  ...
  // The cases above carry no positive signal, so they fall under the
  // threshold even without the negative patterns. These would score as
  // Markdown on their list items alone and are rejected only by subtraction.
  const outweighed: [string, string][] = [
    ["html list", "- <b>one</b>\n- <i>two</i>"],
    ["email with a list body", "From: a@b.com\nTo: c@d.com\nSubject: hi\n\n- one\n- two"],
  ];
  it.each(outweighed)("rejects despite list syntax: %s", (_name, text) => {
    expect(isProbablyMarkdown(text)).toBe(false);
  });
});
