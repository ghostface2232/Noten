...
describe("corpus is in canonical form", () => {
  it.each(CORPUS)("normalize is identity and the text equals itself for: %s", (_name, text) => {
    expect(normalizeMarkdown(text)).toBe(text);
    expect(markdownEqual(text, text)).toBe(true);
  });

  it("normalize is idempotent on a non-canonical input", () => {
    const messy = BOM + "x\r\ny\r\n\n";
...
describe("multibyte and whitespace safety", () => {
  ...
  it("keeps interior list-nesting indentation significant", () => {
