...
1. **노트를 연 직후 첫 `getMarkdown`**(1MB 약 120ms): 유휴 시간에 미리 채우면 첫 자동저장의 멈춤도 사라집니다(`requestIdleCallback`).
2. **파싱 실패 방어**: `openDocument`가 예외로 끝나지 않게 하고 원문을 읽기 전용으로 보여 주는 것입니다. 알려진 재현 입력은 고쳤고, 파서 퍼즈 2,000건에서 예외가 없어 보류했습니다.
3. **코드블럭 NodeView 경량화**: 버튼과 SVG를 템플릿으로 만드는 것입니다. code-1m 로딩 2.6초 중 약 160ms라 효과가 작습니다.
4. **거대 단일 문단 인라인 렉싱**: 원인은 Underline `start`의 `indexOf("++")`입니다(§7.5). 결과가 같다고 보장하기 어려워 보류했습니다.
5. **구조적 한계라 보류**:
