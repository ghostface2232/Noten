...
- **검증**: `npm run check` 통과(68파일 1,229개), Rust `cargo test --lib` 4개 통과.
- **작업 절차**:
...
- **브랜치 전체 교차 리뷰(역할 분할 2개)**: 반영한 사항은 다음과 같습니다.
  - A(에디터 동작):
    - 문서 전체 교체 시 NodeView 요소가 밀리는 문제 → 변경 범위를 노드 identity 기준으로 검출합니다.
    - DPI 변경 시 재측정합니다.
    - 워밍에서 편집 중인 블록을 제외합니다.
  - B(보안·데이터):
    - PDF 이미지를 순차로 인라인하고, 누락되면 경고합니다.
    - 해석 불가 URL을 제거하고, 넓은 표를 인쇄 가능하게 했습니다.
    - 404 응답을 캐시하지 않습니다.
    - 문서를 정정했습니다.
- **개요 점프 경합(기존 버그 포함)**: 목표 위치를 애니메이션 시작 때 한 번만 계산하던 것이 원인이었습니다.
  - 패널 폭 전환 중 레이아웃이 바뀌면 1MB 혼합 노트에서 수천 px 빗나갔습니다(수정 전 빌드 2/10, 기존 빌드에서도 1회 관찰).
  - 매 프레임 목표를 다시 읽도록 바꿨고, 20/20 모두 착지했습니다(`7336f53`). 빗나간 시점에 블록 크기는 모두 정확했습니다(`BENCH_NAV_DIAG`로 확인).
  - 남은 차이: 패널을 연 직후 첫 점프는 목표보다 32px 위(48px 대 80px)에 멈춥니다. 제목은 화면 안에 있습니다.
